...
execute unless data storage uws:temp {player:{deactivate_condition:["taken_hit"]}} unless data storage uws:temp {player:{deactivate_condition:["taken_crit_hit"]}} run return fail
execute if data storage uws:temp {player:{deactivate_condition:["taken_crit_hit"]}} on attacker unless predicate uws:can_critical_hit run return fail
