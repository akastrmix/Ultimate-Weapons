...
execute if score #temp_has_nether_wart uws.state matches 0 run return fail
execute unless block ~ ~-1 ~ minecraft:soul_sand run return fail
...
scoreboard players set #temp_clear_nether_wart uws.state 1
