...
execute if data storage uws:temp {block_data:{force_origin_view:1b}} run tp @s ~ ~ ~ ~ ~
execute unless data storage uws:temp {block_data:{force_origin_view:1b}} run tp @s ~ ~ ~
...
execute unless entity @s[tag=!uws.use_actionbar_ready_notifications,tag=!uws.use_chat_ready_notifications] run function uws:libraries/effect/attempt_ready_notification {effect:Anchor}
